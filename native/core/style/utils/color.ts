...
function rgbToRgb(rgb: string) {
  ...
  throw new Error("Invalid rgb/rgba color code");
}
...
export class Color {
  ...
  constructor(color: string | number) {
    if (typeof color === "string") {
      if (color.startsWith("#")) {
        ...
        this.parseHsvColor(color);
      } else if (color.startsWith("hsl")) {
        this.parseHslColor(color);
      } else {
        this.parseColorName(color);
      }
    } else if (typeof color === "number") {
      this.parseIntColor(color);
    } else {
  ...
  private parseRgbColor(rgb: string) {
    const { r, g, b, a } = rgbToRgb(rgb);
    this.red = r / 255;
    this.green = g / 255;
    this.blue = b / 255;
    this.alpha = a;
  ...
  private parseHsvColor(hsv: string) {
    ...
    if (hsvMatch) {
      ...
      throw new Error("Invalid hsv color code");
    }
  }

  private parseHslColor(hsl: string) {
  ...
  private parseIntColor(color: number) {
  ...
  public fromNSColor(color: NSColor) {
    this.red = color.redComponent;
    this.green = color.greenComponent;
